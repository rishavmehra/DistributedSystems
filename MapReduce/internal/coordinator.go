...
func (c *Coordinator) UpdateTaskStatus(args *MarkTaskAsDoneArgs, reply *MarkTaskAsDoneReply) error {
	c.cond.L.Lock()
...
func (c *Coordinator) GetTask(args *GetTaskArgs, reply *GetTaskReply) error {
	...
	if c.rRemaining != 0 {
		...
		for rTask == "" {
			if c.rRemaining == 0 {
				c.cond.L.Unlock()
				return errors.New("task completed, no more task")
			}
			c.cond.Wait()
			rTask, rNumber = c.getRTask()
		}
		...
		if c.mRemaining != 0 {
			for task := range c.mTasks {
				...
				if status == inprogress {
					...
					if diff > 0 {
						log.Printf("reschedule task with name '%s', type '%s'", task, mType)
